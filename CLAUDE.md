...
- **Round 21 Phase C (remaining piece) — no historical per-milestone cross-chain signal exists for a device that didn't submit the release itself.** `MilestoneRow`'s `historicallyCrossChain(cctpTrack, trackingDomain)` closes the visibility bug (a later `updateReceivingAddress`/`updateSplitReceivingAddress` redirect on an active escrow hiding an already-tracked cross-chain release) only for the device that originally submitted it — `cctpTrack`'s presence in that device's own localStorage is proof-positive of history, independent of what the config says now. A different device (the counterparty, or this one after the 24h localStorage eviction) has no local record and falls back to `trackingDomain`, current-config-derived and therefore still wrong post-redirect. Confirmed via direct trace of `indexer/schema.graphql` and `indexer/src/mapping.ts`: no historical domain signal exists to fall back on instead — `Milestone.releaseTx` is a bare tx hash with no domain field; none of the 5 release-type contract events (`MilestoneApproved`, `MilestoneReleased`, `DisputeResolved`, `DisputeTimedOutSettled`, `MutualSettlementExecuted`) carry domain/mintRecipient data in their signatures; `ReceivingAddressUpdated`/`SplitReceivingAddressUpdated` overwrite the live field in place with — per the mapping's own comment — "no history entity"; `CrossChainLegCreditedOnArc` is the opposite signal (fires only on a too-small-to-burn divert, not a normal cross-chain release). Two real paths to closing it fully, not chosen here since it's a scoping decision for whenever this gets prioritized: (1) a subgraph schema/mapping change adding a domain-at-release-time field (a new indexer version), or (2) frontend-side RPC decoding of the release tx's receipt for a CCTP burn log, independent of the indexer entirely.
