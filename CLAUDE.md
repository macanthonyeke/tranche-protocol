...
Every wallet-signing site in the frontend carries a `confirm` descriptor (the object Circle's User-Controlled Wallet screen is built from). Counted from source rather than from memory — three different, all-legitimate ways to count, so name which one you mean:
...
- **Expanded call-site paths**: how many confirm-descriptor SCREENS exist once the 2 variable-function sites are expanded across every value they can resolve to — **32** (25 static/quoted call sites, each its own screen to review, + 4 milestone actions `action.fn` can resolve to + 3 evidence modes `meta.fn` can resolve to). This is a coverage metric, not a function count: `resolveDisputeByTimeout`'s two call sites (`EscrowDetail.jsx` and `ArbiterPanel.jsx`) are two of the 25 static ones, each with its own descriptor to get right, so it is deliberately NOT deduplicated here the way the lexical count above explicitly avoids doing either.
- **Unique contract functions**: how many DISTINCT contract function names are reachable at all — **31** (24 unique quoted functions, since `resolveDisputeByTimeout` is 2 of the 25 static call sites but only 1 function name, + 4 milestone functions + 3 evidence functions). This is the right number for "how many contract functions have a descriptor," not "how many screens exist" — say which you mean.
...
- Round 34 (PR #18, 22nd review pass) — `useAuth.jsx`'s `executeContractCall` (`useAuth.jsx:378-410`) has no single-flight guard around the shared `sdkRef` SDK singleton. Two components can each start a Circle transaction before the first one's confirm iframe is ready: the second call's `applyConfirmLocalization`/`sdk.execute()` overwrites the singleton's mutable challenge/descriptor/callback state, so the first action's frame can end up showing the second action's confirmation screen while the first action's promise never resolves. This matters more than an ordinary race because it undermines the confirm-descriptors' core guarantee (the screen shown matches the action being signed) via a timing window rather than a wording flaw — the exact property this whole descriptor sweep exists to protect. Confirmed pre-existing in the base branch (`feature/circle-ucw-wallets`), not introduced by any of the 33 rounds on `feature/confirm-descriptors`. Deferred rather than fixed here: (a) it predates everything this PR has touched, and (b) there's a concrete plan to replace Circle's popup entirely with a custom confirmation UI, which will restructure or eliminate this exact execution path — hardening code that's about to be replaced isn't worth it. Suggested fix for whoever picks this up (likely during the custom-UI work): a global in-flight guard in `AuthProvider`, held from before challenge creation until the SDK callback settles, so a second `executeContractCall` either queues or rejects instead of clobbering the first.

Deferred from Round 13 Phase D — investigated, scoped, deliberately not built:

- **Round 13 #17 — the protocol-fee snapshot is reachable, but not from where the descriptors read.** `escrowFeeBps` is emitted as `EscrowTermsSnapshotted` (`TrancheProtocol.sol:350`), indexed at `indexer/src/mapping.ts:208` into `Escrow.protocolFeeBps` (`indexer/schema.graphql:42`). So the "no getter, therefore gross only" premise in `mutualSettleConfirm`, `resolveDisputeConfirm`, `timeoutSettlementConfirm` and `payoutLines` is now obsolete *as a data-availability claim*. What blocks it is plumbing, not availability: those four descriptor families live on `EscrowDetail`/`ArbiterPanel`, whose `escrow` comes from the on-chain `getEscrowDetail`, and the only per-escrow Goldsky read on those pages (`fetchEscrowInvoice` → `useEscrowInvoice`) is gated on the escrow having an invoice at all three call sites, so piggybacking there would deliver the fee for invoiced escrows only. Doing it properly needs a new ungated per-escrow query/hook (`escrow(id) { protocolFeeBps }`), threading into the four families, an honest-degradation path when `VITE_GOLDSKY_ENDPOINT` is unset, and inverting four existing mutation-verified guards that currently assert no rate and no net ever appears (`ArbiterPanel.confirm.test.jsx:166-168`, `ArbiterPanel.resolve.test.jsx:82-89`, `EscrowDetail.settle.test.jsx:110-118`, plus `EscrowDetail.confirm.test.jsx:158`'s rationale comment). Pairs naturally with SE-3, which needs a per-escrow subgraph read of its own.
- **Round 13 #18.4 — the CCTP forward-fee settings descriptor stays cautionary, deliberately.** `ProtocolSettings.jsx`'s `cctpForwardFeeConfirm` warns that the floor must stay at or above Circle's live fee without quoting one. That is not an oversight: `utils/cctpFee.js`'s `fetchForwardFee(src, dst)` is per-destination-domain and gas-priced, while the floor being set is protocol-wide, so there is no single honest number to attach. Quoting every supported domain and showing the max would be the defensible version (the floor's job is covering the worst route) at the cost of N Iris calls on a settings screen — and it would make this the only async descriptor builder in the sweep, since every other one is a pure sync function built inline at its call site.

Found during the Circle confirm-descriptor sweep on `feature/confirm-descriptors`. All five are missing *behaviour*, not missing copy, which is why none was fixed in that work:
...
- `EscrowDetail.jsx`'s `ReceivingAddressEditRow` / `SplitAddressEditRow`: the "Receiving chain" dropdown offers every supported EVM domain plus Arc, even when the escrow (or split leg) currently pays on Arc — a direction F3 guarantees will revert, because those milestones were never floor-validated against the CCTP forwarding fee at deposit (`MilestoneBelowForwardFee` at `TrancheProtocol.sol:982` for the escrow-level path, which fires only when no splits are configured, and `:1033` per split leg). Cross-chain → cross-chain and anything → Arc stay allowed. The confirm screen now warns before signing — `redirectPayoutConfirm` / `redirectSplitConfirm`'s blocked branch — but the honest fix is filtering the unreachable domains out of `domainOptions`, built identically at `:859` and `:905`. Not done this round because it is a behaviour change, not copy. The same root cause surfaces a second time in the row's own `help` text — "You can update this anytime before the escrow is completed or cancelled" — which is equally unqualified. Both spots invite the user to commit to a redirect F3 will reject, and neither discloses the restriction until the final confirm screen.
