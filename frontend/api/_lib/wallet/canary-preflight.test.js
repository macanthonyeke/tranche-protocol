...
describe('POST /api/wallet/canary-preflight', () => {
  ...
  it('rejects a Circle bearer token whose user is not the session subject', async () => {
    vi.stubEnv('UCW_CANARY_ENABLED', 'true')
    circleMock.getUserStatus.mockResolvedValue({ data: { id: 'different-circle-user' } })

    const res = await invoke({ userToken: 'opaque' })

    expect(res.statusCode).toBe(401)
    expect(res.payload).toEqual({ error: 'Circle credentials do not match the signed-in wallet session.' })
    expect(circleMock.listWallets).not.toHaveBeenCalled()
  })
