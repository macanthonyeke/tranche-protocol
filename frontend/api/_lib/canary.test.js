...
describe('UCW canary identity and read-only gates', () => {
  it('requires an exact supported scaCore match from the public wallet filter', async () => {
    ...
    const report = await runCanaryPreflight({ circle, publicClient: fakePublicClient(), userToken: 'opaque', expectedIdentity: IDENTITY, now: NOW })
    ...
    expect(report).not.toHaveProperty('userToken')
  ...
  it('requires the complete canonical identity from the authenticated session', async () => {
    const circle = fakeCircle()

    await expect(runCanaryPreflight({
      circle,
      publicClient: fakePublicClient(),
      userToken: 'opaque',
      now: NOW
    })).rejects.toThrow(/authenticated tranche identity is required/i)
  })

  it.each(['walletId', 'walletAddress', 'blockchain', 'accountType'])(
    'fails closed when the Circle wallet %s differs from the session', async (field) => {
      const circle = fakeCircle()
      const mismatched = {
        ...IDENTITY,
        [field]: field === 'walletAddress'
      ...
      await expect(runCanaryPreflight({
        ...
        now: NOW
      })).rejects.toThrow(/does not match the authenticated tranche session/i)
    }
  )
